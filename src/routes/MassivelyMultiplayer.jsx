...
const MassivelyMultiplayer = () => {
  const [state, dispatch, { fetchData }] = useAppContext();
  ...
  return (
    <Container>
      <Container>
        <Row>
          <Col style={{ fontSize: 40, fontWeight: 700 }}>Multiplayer Game</Col>
        </Row>
      </Container>
      <Filter />
      <Container style={{ paddingTop: 20 }}>
        <InfiniteScroll
          dataLength={games && games.length}
          next={fetchData}
          hasMore={true}
            ...
            {games &&
              games.map((game) =>
                game.genres.map((genres) => {
                  if (genres.name === "Massively Multiplayer") {
                    return (
                      <Col
                        lg={4}
                        md={6}
...
export default MassivelyMultiplayer;
